...
namespace feature_tracker
{
    ...
    FeatureExtraction::FeatureExtraction(std::string module_path,
                                           float image_h,
                                           float image_w)
        : 
          image_h_(image_h),
          image_w_(image_w)
    {
        ...
        cudaMalloc(&buffer_[1], image_h_ * image_w_ / 64 * 256 * sizeof(float));
    }


cv::Mat FeatureExtraction::getDesc(const cv::Mat &img)
{
    cv::Mat image;
    ...
    cudaMemcpy(desc_buffer, buffer_[1], image_h_ * image_w_ / 64 * 256 * sizeof(float), cudaMemcpyDeviceToHost);
...
cv::Mat FeatureExtraction::getDesc1D(const cv::Mat &img)
{
    cv::Mat image;
    img.convertTo(image, CV_32FC1); // or CV_32F works (too)
    image /= 255.;
